init python:
    ...
    def talk_loop_all_charactor(finish_condition, max_turn=5, last_char_line=""):
        global player_name, character_1_affinity, character_2_affinity, character_3_affinity, character_4_affinity, professor_affinity
        global system_prompt_character_1, system_prompt_character_2, system_prompt_character_3, system_prompt_character_4, system_prompt_professor
        emotion_map = {
            "normal": "normal",
            "smile": "smile",
            "sad": "sad",
        ...
        last_speaker = character_2_id
        renpy.show("character_2 normal", at_list=[store.center])
        last_reply = "선배님들은 오늘 끝나고 뭐하세요? 일정 따로 있으세요?"
        renpy.say(character_2, last_reply)

        while True:
            if turn_count >= max_turn+2:
                return

            if last_speaker == character_1_id:
                choice_affinity = character_1_affinity
                choice_prompt = system_prompt_character_1
            elif last_speaker == character_2_id:
                choice_affinity = character_2_affinity
                choice_prompt = system_prompt_character_2
            else:
                choice_affinity = character_3_affinity
                choice_prompt = system_prompt_character_3
            ...
            if charactor == character_1_id:
                ...
                sys_prompt = system_prompt_character_1

            elif charactor == character_2_id:
                current_affinity = character_2_affinity
                sys_prompt = system_prompt_character_2
    ...
    """
        ...
        try:
            ...
            if "candidates" not in result:
                return random.choice([character_1_id, character_2_id])
            ...
            if text.startswith("character:"):
                ...
                if name in [character_1_id, character_2_id]:
                    return name
